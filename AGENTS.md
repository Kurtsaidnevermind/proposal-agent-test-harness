...
- **Regression check after an agent change:** `python scripts/prepare_grading.py --regression`, grade, compile, then compare the new `results/scores.csv` against the most recent dated file in `results/history/` and report per-test deltas. Do not assume git is available; this team works from a shared folder.
...
- **Score history:** `compile_results.py` already writes a dated copy to `results/history/` on every run, so trends accumulate without version control. Never delete that folder.
